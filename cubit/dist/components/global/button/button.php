...
$className = [
  $button_type == 'primary' ? 'bg-teal-default hover:bg-teal-dark text-white' : '',
  $button_type == 'black' ? 'bg-teal-default bg-teal-default text-white md:bg-black-500 md:text-white' : '',
  $button_type == 'outline' ? 'bg-transparent border border border-teal-default text-teal-default hover:bg-teal-default hover:text-white' : '',
  $button_type == 'bg' ? 'bg-seal border text-white font-light hover:bg-teal-dark  border-teal' : '',
...
<?php else: ?>
  <a href="<?= $link ?>" <?php if ($type == 'external'): ?> target="_blank" <?php endif; ?> class="<?= $classString ?>"
    <?= $attr ?>><?= $text ?>   <?= get_svg($icon); ?></a>
<?php endif; ?>
<?php
unset($button);
unset($button_styles);
