...
function replace_cf7_template($form) {
  $components_with_forms = [
    [
      ...
      'index' => 'form.short_code',
      'path' => 'components/page/contact-us/form',
    ],

    [
      'global_field_name' => 'choose_service_form',
      'index' => 'form.shortcode',
      'path' => 'components/page/choose-us/form',
    ],

  ];
  $shortcodes = array_merge(...array_map(function ($component) {
    if (!isset($component['index'])) {
      $component['index'] = 'short_code';
    }
...
function imp_wpcf7_form_elements( $content ) {
    $str_pos = strpos( $content, 'name="service_requested"' ); //name='yourfield name'
    if ( $str_pos !== false ) {
        $content = substr_replace( $content, ' $ref="service_requested" ', $str_pos, 0 );
    }
    return $content;
}